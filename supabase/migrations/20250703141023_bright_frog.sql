/*
  ...
  1. New Functions
    - `sync_participant_count`: Securely updates class participant counts
    - `update_teacher_rating_secure`: Updates teacher ratings based on reviews
    - `create_test_booking`: Creates test bookings with proper count management
    - `save_teacher_for_student`: Securely saves teachers for students
    - `create_teacher_review`: Securely creates teacher reviews

  2. Security
    - Fixed RLS policies for bookings, saved_teachers, teacher_ratings, and teacher_reviews
    - Added proper permission grants for secure functions
    - Created trigger for automatic participant count synchronization

  3. Data Integrity
    - Added code to ensure all classes have correct participant counts
*/
...
-- Fix teacher_reviews RLS policies - check if policies exist before dropping
DO $$
BEGIN
  -- Drop "Students can create and manage their reviews" if it exists
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'teacher_reviews' 
    AND policyname = 'Students can create and manage their reviews'
  ) THEN
    DROP POLICY "Students can create and manage their reviews" ON teacher_reviews;
  END IF;
  
  -- Drop "Students can create their own reviews" if it exists
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'teacher_reviews' 
    AND policyname = 'Students can create their own reviews'
  ) THEN
    DROP POLICY "Students can create their own reviews" ON teacher_reviews;
  END IF;
  
  -- Drop "Students can view and update their own reviews" if it exists
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'teacher_reviews' 
    AND policyname = 'Students can view and update their own reviews'
  ) THEN
    DROP POLICY "Students can view and update their own reviews" ON teacher_reviews;
  END IF;
  
  -- Drop "Anyone can view teacher reviews" if it exists
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'teacher_reviews' 
    AND policyname = 'Anyone can view teacher reviews'
  ) THEN
    DROP POLICY "Anyone can view teacher reviews" ON teacher_reviews;
  END IF;
END
$$;

-- Create teacher_reviews policies
