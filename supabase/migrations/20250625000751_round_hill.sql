/*
  # Fix Database Functions and Testing Issues

  1. Database Functions
    - Drop and recreate functions to avoid return type conflicts
    - Create booking management functions with participant count handling
    - Add validation and utility functions for testing
  ...
  3. Testing Support
    ...
    - Add utility functions for database validation
    - Enable proper CRUD operations for all test scenarios

  4. Security
    - Maintain data integrity with proper constraints
    - Keep audit logging for all participant count changes
...
CREATE INDEX IF NOT EXISTS idx_yoga_classes_date_future ON yoga_classes(date) WHERE date >= CURRENT_DATE;
