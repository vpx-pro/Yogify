/*
  ...
  1. New Tables
    - `participant_count_audit` - Tracks all participant count changes with detailed logging
  
  2. Functions
    - `log_participant_count_change()` - Logs count changes for audit trail
    - `increment_participant_count()` - Safely increments participant count with validation
    - `decrement_participant_count()` - Safely decrements participant count (never below 0)
    - `sync_participant_count()` - Synchronizes count with actual bookings
    - `create_booking_with_count()` - Creates booking with atomic count management
    - `cancel_booking_with_count()` - Cancels booking with atomic count management
    - `validate_all_participant_counts()` - Validates and fixes all class counts
  
  3. Triggers
    - `handle_payment_status_change()` - Automatically adjusts counts when payment status changes
